...
#' @export genotype table infile of SNPs
#' @return nothing
#'
#' @details
...
#' @export Apparent infile of SNPs
#' @return nothing
#'
#' @details
                         ...
                         out_file = "related_infile.txt")
{
  ...
  gt <- extract.gt(vcf2, return.alleles = T, convertNA = T) #convertNA not working here
  gt[gt == "."] <- "NA\tNA"
...
vcf_add_indivs <- function(vcf, vcf1, indiv, whitelist = TRUE) {
  ...
  ids <- ids+1
  if (whitelist == TRUE){
    vcf_merged_gt <-left_join(vcf_gt, vcf1_gt[, c(1,ids,ncol(vcf1_gt))]) %>%
      select(-id) %>%
      as.matrix()
  }
  else {
    vcf_merged_gt <-left_join(vcf_gt, vcf1_gt[, -c(ids)]) %>%
      select(-id) %>%
      as.matrix()
  }
  vcf@gt <- vcf_merged_gt
